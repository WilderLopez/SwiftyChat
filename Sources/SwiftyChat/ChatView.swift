...
public struct ChatView<Message: ChatMessage, User: ChatUser>: View {
    ...
    private var onRemoteResponse: (MockMessages.RemoteResponseRow) -> Void = { item  in print("message \(String(describing: item.message?.securityID)) already donwloaded: \(item.isdownloaded ?? false)")}
...
public extension ChatView {
    ...
    func onRemoteResponse(action: @escaping (MockMessages.RemoteResponseRow) -> Void) -> Self {
