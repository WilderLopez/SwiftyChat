...
public enum ImageLoadingKind {
    case local(UIImage)
    case remote(URL)
    case remoteImage(UIImage, URL, CGSize, Double, Bool)
}
...
public enum ChatMessageKind: CustomStringConvertible {
    ...
    public var description: String {
        ...
        case .image(let imageLoadingType):
            ...
            case .remoteImage(let tnail, let url, let size, let tnailByte, let isRemote):
                return "MessageKind.image(remoteTodus:\(tnail), \(url) , \(size), \(tnailByte), \(isRemote)"
            }
        case .text(let text):
